...
/* If this is set, all individuals (even ones with no parents) may receive
phenotype information */
#define ALL_KIDS_PHENO	0

print_pheno_row(PEDIGREE *p, char *Ticket,int generation)
{
int i=0,j=0;
  make_sub_cell("white","white",DB_FONT_COLOR);
  printf("%s<FONT COLOR=black>",TIMES_10);
  printf("<TABLE BORDER=0 WIDTH=100%%><FORM METHOD=POST ACTION=%s TARGET=_top><INPUT TYPE=HIDDEN NAME=PROCESS_PHENO VALUE=%s><INPUT TYPE=HIDDEN NAME=GENERATION VALUE=%d><TR>",DB_NAME,Ticket,generation);
  for (i=0; i<p->nbindivs; i++)
  {
#if !ALL_KIDS_PHENO
           if ((i<2)||(p->indivs[i]->father>=0))	/* Only if parent */
           {
#endif
		if ((j!=0)&&(j%10==0)) printf("</TR><TR>");
...
INDIV *ind=NULL;
  ...
  c2=getenv("REQUEST_METHOD");
		...
		print_cleartop(1,0,1,"GenPed Error","white","black");
                printf("<FONT COLOR=black>PROBLEM WITH POST[%s]/GET[%s] TRANSMISSION<br>\n",c,c2);
        }
...
/***** PRINT COUPLE0 ROWS OF CHILDREN *****/
	...
	case 4: /* COMPLETE PEDIGREE */
		...
		sprintf(dirname,"%s/data/%s/tmp/%s", GP_ROOT_DIR, DB_NAME, Ticket);
		...
		if (found)
		{
		  ...
		  printf("</FONT></FONT>\n</P>\n");
			...
			else	/* COMPLETED */
			{
			  printf("<TABLE BORDER=0 WIDTH=100%%><TR>");
		  	  printf("<TH BGCOLOR=white WIDTH=10%%>&nbsp;</TH>");
		  	  printf("<TD BGCOLOR=white WIDTH=80%%>");
			  printf("<P ALIGN=JUSTIFY>%s<FONT COLOR=black>Now that you completed your family pedigree, please indicate below which individuals are affected by the %s disorder:\n</P></TD>", TIMES_10,DB_DISORDER_NAME);
		  	  printf("<TH BGCOLOR=white WIDTH=10%%>&nbsp;</TH>");
			  printf("</TR></TABLE>");
			  print_pheno_row(ped,Ticket,generation);
			}
#if 0
	for (i=0; i<ped->nbindivs; i++)
	{
	    ...
	    case 8:
                if (strcpy(Ticket,entries[0].val)==NULL)
                   print_error_page("white", "black", DB_FONT_COLOR, TIMES_12, "Invalid Query ticket");
                if (strcmp(entries[1].name,"GENERATION")!=0)
                   print_error_page("white", "black", DB_FONT_COLOR, TIMES_12, "Invalid Query generation");
                if (sscanf(entries[1].val,"%d",&generation)!=1)
                   print_error_page("white", "black", DB_FONT_COLOR, TIMES_12, "Invalid Form (Generation)");
                sprintf(dirname,"%s/data/%s/tmp/%s", GP_ROOT_DIR, DB_NAME, Ticket);
#if VERBOSE
		...
		sprintf(pedfile,"%s/ped.%d", dirname, generation);
#if VERBOSE
                printf("Your ticket number is [%s].<BR>Pedfile=[%s]<HR>",Ticket,pedfile);
#endif
                if ((ped=load_pedigree(pedfile,0))==NULL)       /* Only Load */
                {
                    sprintf(strtmp,"GenPed Error Code %d (%s)", GP_LOAD_PEDIGREE_ERROR,pedfile);
		...
		i=2;
		while (strcmp(entries[i].name,"PHENO")==0)
		{
		   if (sscanf(entries[i].val,"%d",&j)==1)
		   {
#if VERBOSE
			printf("INDIV%d has %s.<BR>",j,DB_DISORDER_NAME);
#endif
			ped->indivs[j-1]->pheno=1;
			count++;
		   }
		   i++;
		}
		/* WRITE THE PED.PHENO FINAL PEDIGREE FILE */
	 	sprintf(pedfile,"%s/ped.pheno", dirname);
		fd=fopen(pedfile,"w");
                for (i=0; i<ped->nbindivs; i++)
                {
                   ind=ped->indivs[i];
                   fprintf(fd,"%d %d %d %d %d %d %d %d %d\n",
                        ind->id+1, ind->sex, ind->mother+1, ind->father+1,
                        ind->spouse+1, ind->kid+1, ind->sib+1,(int)(ind->proband),ind->pheno);
                }
                fclose(fd);
#if VERBOSE
		printf("There are %d Individuals in this pedigree.<BR>",ped->nbindivs);
#endif
		Sometime2=0;
                Sometime2=time(&Sometime2);
                sprintf(strtmp,"%s/data/%s/tmp/%ld.gif", GP_ROOT_DIR, DB_NAME, Sometime2);
                gif2pedigree(ped, strtmp,1);	/* Draw phenotyped individuals*/
		/* OUTPUT THE NEW FRAMESET */
		printf("Content-Type: text/html\n\r\n\r");
                printf("<HTML><HEAD><TITLE>%s - Copyright(c)1999 GenPed Inc.</TITLE>", DB_HTML_NAME);
                printf("</HEAD>");
                printf("<FRAMESET COLS=\"100%%\" BORDER=0>\n");
                ...
                printf("<FRAME name=PAGETOP src=%s?SHOW_PED_PAGE=%ld scrolling=auto marginwidth=0 marginheight=0 frameborder=0 borderheight=0 borderwidth=0>",DB_NAME, Sometime2);
                if (relat<0)
                  printf("<FRAME name=PAGEBOT src=%s?FINISHED_PHENO=%s", DB_NAME,Ticket);
                printf(" scrolling=auto marginwidth=0 marginheight=0 frameborder=1 borderheight=0 borderwidth=0>");
                printf("<FRAME name=PAGEBOT src=%s?NEXT_FORM=%s&GENERATION=%d scrolling=auto marginwidth=0 marginheight=0 frameborder=1 borderheight=0 borderwidth=0>",DB_NAME, Ticket, generation);
                printf("</FRAMESET>\n");
                printf("</FRAMESET>\n");
	    ...
	    case 9:
                ...
                if (strcpy(Ticket,entries[0].val)==NULL)
                   print_error_page("white", "black", DB_FONT_COLOR, TIMES_12, "Invalid Query ticket");
		printf("<FORM METHOD=POST ACTION=%s TARGET=_top><INPUT TYPE=HIDDEN NAME=FORM0 VALUE=%s>", DB_NAME,Ticket);
		form2html("/home/genped/data/egdp/etc/form0.conf");
		printf("<INPUT TYPE=SUBMIT NAME='Please Submit this Questionnaire' VALUE=SUBMIT>");
		printf("</FORM>");
		break;
	    case 10:
                sprintf(title,"%s - Copyright(c)1996,99 GenPed", DB_HTML_NAME);
		...
		if (strcpy(Ticket,entries[0].val)==NULL)
                   print_error_page("white", "black", DB_FONT_COLOR, TIMES_12, "Invalid Query ticket");
		printf("<FORM METHOD=POST ACTION=%s TARGET=_top><INPUT TYPE=HIDDEN NAME=PROCESS_USERINFO VALUE=%s>", DB_NAME,Ticket);
		form2html("/home/genped/data/egdp/etc/userform.conf");
		printf("<CENTER><INPUT TYPE=SUBMIT NAME=SUBMIT VALUE='Submit Personal Information'>");
		printf("</FORM>");
		exit(0);
	    case 11:
                sprintf(title,"%s - Copyright(c)1996,99 GenPed", DB_HTML_NAME);
		...
		if (strcpy(Ticket,entries[0].val)==NULL)
                   print_error_page("white", "black", DB_FONT_COLOR, TIMES_12, "Invalid Query ticket");
		sprintf(dirname,"%s/data/%s/tmp/%s", GP_ROOT_DIR, DB_NAME, Ticket);
		sprintf(dirname2,"%s/data/%s/spool/%s", GP_ROOT_DIR, DB_NAME, Ticket);
		if (rename(dirname,dirname2)<0)
		  printf("The current pedigree information seems to have already been saved. You can now <A HREF=javascript:window.close()>close this window</A>");
		else
		  printf("Thank you for submitting your pedigree to the Parkinson Gene Discovery Project [%s]", Ticket);
		exit(0);
	    default:
