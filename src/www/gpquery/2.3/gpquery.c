...
#define GP_MAX_RELATIONS	5
char relation[GP_MAX_RELATIONS][255]= {
...
int i=0;
  ...
  close_cell();
  printf("<CENTER><INPUT TYPE=IMAGE BORDER=0 SRC=%s/gifs/nextpage.jpg ALT=\"Click to proceed...\" onClick=\"document.forms[0].submit()\">",GP_HTDOCS_DIR);
  printf("</FORM>");
}
...
int i=0;
  ...
  printf("%sCouple(%d,%d) has ",HTML_TAB,mother,father);
  printf("<SELECT TYPE=MENU NAME=C_D>");
  for (i=0; i<=MAX_KIDS; i++)
  ...
  for (i=0; i<=MAX_KIDS; i++)
        printf("\t<OPTION VALUE=%d>%d</OPTION>", i,i);
  printf("</SELECT> sons without child(ren).<BR>");
}
...
SION<br>\n",c,c2);
	...
	nb_option=get_pair_option(entries[0].name);
  	switch(nb_option)
  	{
		...
		printf("%s<A HREF=%s?WHATISPEDIGREE=1 TARGET=MAINFRAME><FONT COLOR=#209090>What is a Pedigree ?</FONT></A></FONT>",TIMES_14,DB_NAME);
		printf("<P>");
		printf("%s<A HREF=%s?BUILDPEDIGREE=1 TARGET=MAINFRAME><FONT COLOR=#209090>Build Your Own Family Pedigree</FONT></A></FONT>",TIMES_14,DB_NAME);
		printf("</CENTER>");
		exit(0);
		break;
    	case 1:    /* ACTIVETOP */
		...
		printf("<TH BGCOLOR=WHITE WIDTH=100%%>%s<FONT COLOR=#209090>%s</FONT></FONT></TH>",TIMES_14,DB_HTML_NAME);
		printf("</TR></TABLE>");
		close_cell();
		printf("</BODY></HTML>");
		break;
		...
		print_cleartop(1,0,1,"Building a pedigree","black","white");
		make_sub_cell("white","white",DB_FONT_COLOR);
		printf("<CENTER>%s<FONT COLOR=#209090><I>Powered by <A HREF=http://www.genped.com>GenPed</A>. Copyright (c)1999 </I></FONT></FONT></CENTER></BODY></HTML>",TIMES_12);
		close_cell();
		exit(0);
		break;
		...
		printf("<CENTER><BR><P>%s<FONT COLOR=black>How To Build Your Family Pedigree</FONT></FONT></CENTER>",TIMES_14);
		pid=getpid();
		...
		printf("<INPUT TYPE=HIDDEN NAME=GENERATION VALUE=0>");
 		cache=0;
		printf("<CENTER><HR WIDTH=80%%><TABLE BORDER=0 WIDTH=100%%><TR>");
		printf("<TH BGCOLOR=white WIDTH=10%%>&nbsp;</TH>");
		printf("<TD BGCOLOR=white>");
		printf("%s<FONT COLOR=black><P ALIGN=JUSTIFY>",TIMES_10);
		printf("%sIn order to start adding individuals to your pedigree, you need to identify the oldest living couple in your family <B>where %s can be traced</B>. For example, if you were the oldest member of your family, you would be one of the individuals in this first couple.\n</P>", HTML_TAB,DB_DISORDER_NAME);
		printf("<CENTER><IMG SRC=%s/gifs/oldestcouple.jpg></CENTER>",GP_HTDOCS_DIR);
		printf("<P ALIGN=LEFT>%s<U>Please choose one of the following options:</U>\n</P>", HTML_TAB);
		printf("<CENTER><FONT COLOR=white>%s<SELECT TYPE=MENU NAME=RELATION>",HTML_TAB);
		/* This represents the maximum generations */
		for (i=0; i<GP_MAX_RELATIONS; i++)
		{
		...
		printf("<CENTER>");
		printf("</CENTER>");
		printf("<HR WIDTH=80%%><TABLE BORDER=0 WIDTH=100%%><TR>");
		printf("<TH BGCOLOR=white WIDTH=10%%>&nbsp;</TH>");
		printf("<TD BGCOLOR=white>");
		printf("%s<FONT COLOR=black><P ALIGN=JUSTIFY>",TIMES_10);
		printf("%sNext, you need to indicate in the form below how many children this first couple has, then proceed to the next generation as instructed...\n</P>",HTML_TAB);
		printf("</TD><TH BGCOLOR=white WIDTH=10%%>&nbsp;</TH>");
		printf("</TR></TABLE><BR>&nbsp;");
#if 0
		printf("<TH BGCOLOR=white COLSPAN=2></B>%s<FONT COLOR=black>Daughter(s)</FONT></FONT></TH>",TIMES_10);
		printf("<TH BGCOLOR=white COLSPAN=2></B>%s<FONT COLOR=black>Son(s)</FONT></FONT></TH>",TIMES_10);
		printf("<TABLE BORDER=0 WIDTH=100%%><TR>");
		printf("<TD BGCOLOR=white WIDTH=10%%>&nbsp;</TD>");
		printf("<TH BGCOLOR=white></B>%s<FONT COLOR=black>with children</FONT></FONT></TH>",TIMES_10);
		printf("<TH BGCOLOR=white></B>%s<FONT COLOR=black>without children</FONT></FONT></TH>",TIMES_10);
		printf("<TH BGCOLOR=white></B>%s<FONT COLOR=black>with children</FONT></FONT></TH>",TIMES_10);
		printf("<TH BGCOLOR=white></B>%s<FONT COLOR=black>without children</FONT></FONT></TH>",TIMES_10);
		printf("</TR><TR>");
		print_couple_row(1,2);
		printf("</TD></TR></TABLE>");
		close_cell();
#endif
		printf("<TABLE BORDER=0 WIDTH=100%%><TR>");
		printf("<TD BGCOLOR=white WIDTH=10%%>&nbsp;</TD><TD BGCOLOR=white>");
		printf("<P ALIGN=JUSTIFY>%s<FONT COLOR=black>",TIMES_8);
		print_couple_row2(1,2);
		printf("\n</FONT></FONT></P>\n");
		printf("</TD><TD BGCOLOR=white WIDTH=10%%>&nbsp;</TD>");
		printf("</TR></TABLE><HR WIDTH=80%%><BR>");

		printf("<INPUT TYPE=IMAGE BORDER=0 SRC=%s/gifs/updatepedigree.jpg ALT=\"Click to proceed to the next generation...\" onClick=\"document.forms[0].submit()\">",GP_HTDOCS_DIR);
		printf("</FORM>");
		printf("</BODY></HTML>");
		exit(0);
		break;
	case 4: /* COMPLETE PEDIGREE */
		...
		i=1;
                ...
                printf("<HTML><HEAD><TITLE>%s - Copyright(c)1999 GenPed Inc.</TITLE>", DB_HTML_NAME);
                printf("</HEAD>");
                printf("<FRAMESET COLS=\"100%%\" BORDER=1>\n");
#if 0
                printf("<FRAME name=LEFTFRAME src=%s/noframe.html noresize scrolling=no marginwidth=0 marginheight=0 frameborder=no borderheight=0 borderwidth=0>",GP_HTDOCS_DIR);
#endif
                ...
                printf("<FRAME name=PAGEBOT src=%s?NEXT_FORM=%s&GENERATION=%d",
			DB_NAME,Ticket,generation);
		...
		if (found)
		{
		  printf("<FORM METHOD=GET ACTION=%s TARGET=MAINFRAME>\n", DB_NAME);
		  printf("<INPUT TYPE=HIDDEN NAME=TICKET VALUE=%s>\n", Ticket);
		  printf("<INPUT TYPE=HIDDEN NAME=GENERATION VALUE=%d>\n",generation);
		  printf("<CENTER><BR>");
			  printf("<P>%s<FONT COLOR=black>We now need to add new individuals to the ",TIMES_10);
			  switch(generation%10+1)
				{
				   case 1:
				       printf(" <U>1st generation</U> ",generation+1);
					break;
				   case 2:
				       printf(" <U>%dnd generation</U> ",generation+1);
					break;
				   case 3:
				       printf(" <U>%drd generation</U> ",generation+1);
					break;
				   default:
				       printf(" <U>%dth generation</U> ",generation+1);
					break;
				}
			  printf("of your family pedigree above.<BR>Please indicate how many children each couple listed below has:\n</FONT></FONT><P>");
			make_sub_cell("white","white",DB_FONT_COLOR);
			printf("&nbsp;</TD>");
			printf("<TH BGCOLOR=white COLSPAN=2></B>%s<FONT COLOR=black>Daughter(s)</FONT></FONT></TH>",TIMES_10);
			printf("<TH BGCOLOR=white COLSPAN=2></B>%s<FONT COLOR=black>Son(s)</FONT></FONT></TH>",TIMES_10);
			...
			printf("</TR><TR>");
			  make_sub_cell("white","white",DB_FONT_COLOR);
			  printf("&nbsp;</TD>");
			  printf("<TH BGCOLOR=white></B>%s<FONT COLOR=black>Daughter(s)<BR>with children</FONT></FONT></TH>",TIMES_10);
			  printf("<TH BGCOLOR=white></B>%s<FONT COLOR=black>Daughter(s)<BR>without children</FONT></FONT></TH>",TIMES_10);
			  printf("<TH BGCOLOR=white></B>%s<FONT COLOR=black>Son(s)<BR>with children</FONT></FONT></TH>",TIMES_10);
			  printf("<TH BGCOLOR=white></B>%s<FONT COLOR=black>Son(s)<BR>without children</FONT></FONT></TH>\n\n",TIMES_10);
	#endif
			  ...
			  for (i=0; i<ped->nbindivs; i++)
			  {
				...
				if (ind->spouse>=0)
				{
				  ...
				  if (ind->sex==0)
				  {
				    print_couple_row(i+1,ind->spouse+1);
				    dones[i]=1; dones[ind->spouse]=1;
				  }
				  else
				  {
				    print_couple_row(ind->spouse+1,i+1);
				    dones[i]=1; dones[ind->spouse]=1;
				  }
				}
			      }
			  ...
			  printf("<TD BGCOLOR=white ALIGN=LEFT>%s<FONT COLOR=black><INPUT TYPE=CHECKBOX NAME=SCALE> Scale picture width by<INPUT TYPE=TEXT SIZE=3 NAME=RATIO VALUE=100>%% </FONT></FONT></TD>",TIMES_10);
			  printf("<TD BGCOLOR=white ALIGN=RIGHT>%s<FONT COLOR=black>Cache Next Page <INPUT TYPE=CHECKBOX NAME=CACHE></FONT></FONT></TD>",TIMES_10);
			  printf("</TR></TABLE>");
	#endif
			printf("<BR><CENTER><INPUT TYPE=IMAGE BORDER=0 SRC=%s/gifs/updatepedigree.jpg ALT=\"Click to proceed to the next generation in your family pedigree...\"> \n",GP_HTDOCS_DIR);
			  printf("</P>");
	#if 0
