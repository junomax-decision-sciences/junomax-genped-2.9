...
SION<br>\n",c,c2);
	...
	nb_option=get_pair_option(entries[0].name);
  	switch(nb_option)
  	{
		...
		printf("%sIn order to start your pedigree, you need first to identify the oldest living couple in your family <B>where %s can be traced</B>.\n</P>", HTML_TAB,DB_DISORDER_NAME);
		printf("</FONT></FONT>");
		printf("</TD>");
		printf("<TH BGCOLOR=white WIDTH=10%%>&nbsp;</TH>");
		printf("</TH>");
		...
		printf("</TABLE>");
		printf("<CENTER>");
		printf("<IMG SRC=%s/gifs/Pedigree1.jpg>",GP_HTDOCS_DIR);
		printf("</CENTER>");
		printf("<TABLE BORDER=0 WIDTH=100%%><TR>");
		printf("<TH BGCOLOR=white WIDTH=10%%>&nbsp;</TH>");
		printf("<TD BGCOLOR=white>");
